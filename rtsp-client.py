...
TRANSPORT_TYPE_MAP  = {
                        ...
                        'satip'         :   'RTP/AVP;%s;destination=%s;client_port=%s'
                      }
...
class RTSPClient(threading.Thread):
    ...
    def _process_response(self,msg):
        ...
        if self._cseq_map[rsp_cseq] == 'DESCRIBE':
            ...
            print headers
            self.track_id_str = "stream=" + headers['com.ses.streamid']
    ...
    def _get_transport_type(self):
        ...
        for t in TRANSPORT_TYPE_LIST:
            if t not in TRANSPORT_TYPE_MAP:
                raise Exception('Error param: %s'%t,RED)
                sys.exit(1)
            if t.endswith('tcp'):
            ...
            print(transport_str)
        print(CLIENT_PORT_RANGE, transport_str,CLIENT_PORT_RANGE)
    ...
    def do_describe(self):
        ...
        self._sendmsg('DESCRIBE',self._orig_url.geturl(),headers)

    def do_setup(self):
        headers = {}
        headers['Transport'] = self._get_transport_type()
        print(self._orig_url)
        self._orig_url = self._orig_url._replace(path = '/' + self.track_id_str)
        print(self._orig_url, headers)
        self._sendmsg('SETUP',self._orig_url.geturl(),headers)
    ...
    def do_play(self,range='npt=end-',scale=1):
        ...
        headers['Scale'] = scale
        self._orig_url = self._orig_url._replace(path = '/' + self.track_id_str)
        self._sendmsg('PLAY',self._orig_url.geturl(),headers)

    def do_pause(self):
        self._sendmsg('PAUSE',self._orig_url,{})

    def do_teardown(self):
        self._sendmsg('TEARDOWN',self._orig_url.geturl(),{})
        self.running = False

    def do_options(self):
        self._sendmsg('OPTIONS',self._orig_url.geturl(),{})

    def do_get_parameter(self):
        self._sendmsg('GET_PARAMETER',self._orig_url.geturl(),{})
...
if __name__ == '__main__':
    ...
    parser.add_option('-t','--transport',dest='transport',default='satip',help='Set transport type when SETUP: ts_over_tcp, ts_over_udp, rtp_over_tcp, rtp_over_udp[default]')
